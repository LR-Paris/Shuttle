...
export default function CollectionPage({ params }: { params: Promise<{ collectionId: string }> }) {
  ...
  const sortedProducts = [...collection.products].sort((a: Product, b: Product) => {
    switch (sortBy) {
      case 'name-asc':
        return (a.variantGroup || a.name).localeCompare(b.variantGroup || b.name);
      case 'name-desc':
        return (b.variantGroup || b.name).localeCompare(a.variantGroup || a.name);
  ...
  // Deduplicate: for variant groups, only show the first variant (index 0 in sorted variants array)
      ...
      {collectionImages.length > 0 && hasShowcaseImage ? (
        ...
        <div className="relative w-full h-[400px] mb-8 overflow-hidden bg-gray-100">
          {collectionImages.map((image: string, index: number) => (
            ...
            >
              <img src={image} alt={`Collection item ${index + 1}`} className="w-full h-full object-contain" />
            </div>
          ))}
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end justify-center pb-8">
            <h1 className="text-5xl font-bold text-white px-4 text-center" style={{ fontFamily: design.fonts.titleFont }}>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              style={{ borderColor: design.colors.border, borderRadius: `${cornerRadius}px`, fontFamily: design.fonts.bodyFont, color: design.colors.text }}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {displayProducts.map((product: Product) => {
            // For variant groups, link to the first variant's product page
            ...
            return (
              <Link
                ...
                style={{ borderColor: design.colors.border, borderRadius: `${cornerRadius}px` }}
              >
                {product.images.length > 0 ? (
                  <div className="aspect-square bg-gray-100 relative border-b" style={{ borderColor: design.colors.border }}>
                    <img src={product.images[0]} alt={displayName} className="w-full h-full object-contain p-4" />
                  </div>
                ) : (
                ...
                <div className="p-4">
                  ...
                  {/* Variant chips — show first dimension values */}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {product.variants.map((v: ProductVariant) => (
                          <span
                            ...
                            style={{
                              borderColor: v.id === product.id ? design.colors.secondary : design.colors.border,
                              color: v.id === product.id ? design.colors.secondary : design.colors.textLight,
